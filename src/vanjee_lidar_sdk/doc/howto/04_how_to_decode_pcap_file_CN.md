...
要播放PCAP文件，编译 ```vanjee_lidar_sdk ```时，需要将```DISABLE_PCAP_PARSE```选项设置为```OFF```.
...
lidar:
  - driver:
      ...
      angle_path_hor: "<PROJECT_PATH>/src/vanjee_lidar_sdk/param/Vanjee_720_HA.csv"          # 水平角度配置文件地址
...
设置 ```angle_path_hor```       为绝对路径"<PROJECT_PATH>/src/vanjee_lidar_sdk/param/Vanjee_720_HA.csv", 使用该路径下水平角度表参与点云计算。
