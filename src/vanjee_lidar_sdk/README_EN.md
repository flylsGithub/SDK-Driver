...
- vanjee_722
- vanjee_722d
- vanjee_722f (WLR-722F, WLR-722G Ethernet version)
- vanjee_722h (WLR-722H, WLR-722T, WLR-722X)
- vanjee_722z (WLR-722G Serial version, WLR-722S, WLR-722Z)
- vanjee_732
- vanjee_733
- vanjee_750
- vanjee_760
...
**If the WLR-760 needs to be used, the macro definition "FILTER_DLL_ENABLE=ON" must be enabled during compilation**
**If there is an error message indicating that "libVanJeeLaser760Filter_linux_x86.so" or "libVanJeeLaser760Filter_linux_aarch64.so" cannot be found during compilation or execution, the following steps should be taken**
```sh
sudo cp <PROJECT_PATH>/src/vanjee_lidar_sdk/src/vanjee_driver/libVanJeeLaser760Filter_linux_x86.so /usr/local/lib
sudo ldconfig
```
or
```sh
sudo cp <PROJECT_PATH>/src/vanjee_lidar_sdk/src/vanjee_driver/libVanJeeLaser760Filter_linux_aarch64.so /usr/local/lib
sudo ldconfig
```
**If there is still an error message indicating that "libVanJeeLaser760Filter_linux_x86.so" or "libVanJeeLaser760Filter_linux_aarch64.so" cannot be found during compilation or execution, the following steps should be taken**
To check the permissions of the dynamic library, you can execute the following command.
```sh
ls -la <PROJECT_PATH>/src/vanjee_lidar_sdk/src/vanjee_driver/libVanJeeLaser760Filter_linux_x86.so
sudo chmod 777 <PROJECT_PATH>/src/vanjee_lidar_sdk/src/vanjee_driver/libVanJeeLaser760Filter_linux_x86.so
sudo cp <PROJECT_PATH>/src/vanjee_lidar_sdk/src/vanjee_driver/libVanJeeLaser760Filter_linux_x86.so /usr/local/lib
```
or
```sh
ls -la <PROJECT_PATH>/src/vanjee_lidar_sdk/src/vanjee_driver/libVanJeeLaser760Filter_linux_aarch64.so
sudo chmod 777 <PROJECT_PATH>/src/vanjee_lidar_sdk/src/vanjee_driver/libVanJeeLaser760Filter_linux_aarch64.so
sudo cp <PROJECT_PATH>/src/vanjee_lidar_sdk/src/vanjee_driver/libVanJeeLaser760Filter_linux_aarch64.so /usr/local/lib
```
...
(3) Return to the workspace directory, then execute the following commands to compile and run. If using .zsh, replace the second line with *source install/setup.zsh*.
...
[Parameter Introduction](doc/intro/02_parameter_intro_EN.md)
...
[Connect to online LiDAR data and publish point clouds to ROS](doc/howto/02_how_to_decode_online_lidar_EN.md)

[Parse PCAP packets and publish point clouds to ROS](doc/howto/04_how_to_decode_pcap_file_EN.md)

[Switch point types](doc/howto/01_how_to_change_point_type_EN.md) 
