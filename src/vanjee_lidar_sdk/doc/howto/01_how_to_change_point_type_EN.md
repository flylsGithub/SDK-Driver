...
When `POINT_TYPE` is set to `XYZIRT`, the `vanjee_lidar_sdk` utilizes the custom point type `PointXYZIRT` defined by Vanjee.
...
When `POINT_TYPE` is set to `XYZIRTT`, the `vanjee_lidar_sdk` utilizes the custom point type `PointXYZIRTT` defined by Vanjee.
